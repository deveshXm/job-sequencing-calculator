...
function job_deadline(n) {
  ...
  let max = 0;
  for (var i = 0; i < n; i++) {
    if (parseInt(d[i]) > max) max = parseInt(d[i]);
  }
  sortList(p, d);
...
function maketable(n) {
  ...
  for (var i = 0; i < n; i++) {
    str1 += `<tr><td>Job-${
      i + 1
    }</td><td><input type="number" id="row${1}${i}" placeholder="Enter Profit"><td><input type="number" id="row${2}${i}" placeholder="Enter Job deadline"></td></tr>`;
  }
...
make.addEventListener("click", () => {
  ...
  maketable(n);
  if (class1.classList.contains("class1")) class1.classList.toggle("class1");
  cal.scrollIntoView();
});
